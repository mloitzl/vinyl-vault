# Vinyl Vault – Architecture

## 1. Overview

Vinyl Vault is a web application for managing vinyl record collections with multi-tenant support. Users can manage personal collections or collaborate within GitHub organization collections. The system scans barcodes on record sleeves, looks up metadata from external music databases, and stores collections in isolated MongoDB databases per tenant. The solution is built as a public-facing system running on a Raspberry Pi 5, with a security-focused design and a Backend-for-Frontend (BFF) proxy.

## 2. Goals and Constraints

- Minimize manual data entry for vinyl records.
- Keep all JWTs and backend secrets away from the browser.
- Use GitHub as the single sign-on provider.
- Support multi-tenant collections (personal and GitHub organizations).
- Support role-based access per tenant: ADMIN/MEMBER and VIEWER.
- Provide strict tenant isolation with database-per-tenant architecture.
- Run on a single RPi 5 with limited resources but public internet exposure.
- Prefer clear, maintainable TypeScript code across backend and frontend.
- Use pnpm workspaces for monorepo management.

## 3. Logical Architecture

Main components:

- **React/Relay Web Client** (`packages/frontend`)
  - Vite-powered SPA served over HTTPS.
  - Talks only to the BFF GraphQL endpoint.
  - Uses Tailwind CSS for styling.
  - AuthContext manages user state and current tenant selection via `/auth/me` endpoint.
  - Tenant switcher component allows users to switch between personal and organization collections.
  - Barcode scanning via @zxing/browser library (camera + manual input).
  - Mobile-first design with bottom navigation for authenticated users.
  - Views: Home (welcome), Scan (barcode lookup with album results), Collection (paginated list), Browse (`/browse` with Artists/Albums/Genres tabs), Tenant Selector.
  - Relay `@connection(key: "RecordList_records")` with `usePaginationFragment` for cursor-based "Load More" pagination; `@deleteEdge`/`@appendNode` for instant store updates without network refetch on delete/create.
  - Vite plugin `relayRequireToImport()` in `vite.config.ts` hoists Relay-generated `require()` calls to ESM imports (needed for Relay 16 Rust compiler `@refetchable` output with Vite's ESM dev server).
  - Schema generation: `pnpm update-schema` runs `generate-schema.mjs` which merges static BFF + Backend SDL files into `packages/frontend/schema.graphql` (gitignored); `pnpm relay` then runs relay-compiler; `pnpm build` runs Vite.

- **GraphQL BFF** (`packages/bff`)
  - Express + Apollo Server 4.
  - Exposes a Relay-friendly GraphQL schema via **schema stitching** (`@graphql-tools/stitch`): BFF's own `schema.graphql` contains only auth/session types (`viewer`, `switchTenant`, `Tenant`, `FeatureFlags`); Backend schema fields are automatically proxied at startup by `createStitchedSchema()` which reads the static backend SDL.
  - `packages/bff/src/graphql/executor.ts` builds an HTTP executor via `buildHTTPExecutor` that injects the JWT into every forwarded request.
  - No dual-schema maintenance: adding a field to the Backend schema automatically exposes it through the BFF.
  - Handles GitHub OAuth Web Application Flow:
    - `/auth/github` - Initiates OAuth redirect.
    - `/auth/github/callback` - Exchanges code for token, creates session.
    - `/auth/logout` - Destroys session.
    - `/auth/me` - Returns current user and available tenants.
    - `/auth/tenants` - Lists user's accessible tenants (personal + orgs).
    - `/auth/switch-tenant` - Switches active tenant in session.
    - `/auth/setup` - Handles post-installation redirect from GitHub App to link user and create org tenant.
  - Uses `express-session` with `connect-mongo` for session storage.
  - HTTP-only secure cookies for browser sessions.
  - Session includes current tenant context (tenantId, tenantType).
  - Creates short-lived JWTs with tenant context for calls to the Domain Backend.
  - Receives GitHub App webhook events and integrates with backend for org tenant creation.

- **Domain Backend** (`packages/backend`)
  - Apollo Server (standalone).
  - Encapsulates business logic and data access.
  - Validates JWTs from the BFF (user id, role, tenantId, tenantType).
  - Enforces strict tenant isolation on all database operations.
  - Manages dynamic connections to tenant-specific MongoDB databases.
  - Interacts with tenant MongoDB databases for users, records, and releases.
  - Integrates with external music APIs (Discogs/MusicBrainz) queried in parallel.
  - Implements blended scoring system for release aggregation into Albums.
  - Provides detailed score breakdowns for transparency and debugging.
  - Caches release metadata per tenant to ensure data isolation.
  - Executes faceted full-text search via Typesense: two-step lookup (Typesense for ranking + facets → MongoDB for full document fetch), enforcing tenant isolation server-side on every query.

- **Typesense** (Full-Text Search)
  - Self-hosted 3-node Raft cluster on k3s (staging/production) or Typesense Cloud free tier (demo/Koyeb).
  - Single shared collection `records` with a `tenantId` field; the backend appends `filter_by: tenantId:=<id>` to every query — the browser never accesses Typesense directly.
  - Supports faceted search: counts by artist, title, genre, format, condition, location, country.
  - Highlight snippets surfaced as `SearchHighlight[]` (same GraphQL shape as the previous Atlas Search implementation).
  - Local dev: single-node container via `docker-compose.yml` on port 8108.

- **Sync Worker** (`packages/backend/src/sync-worker/`)
  - Standalone Node process (separate Kubernetes Deployment, `replicas: 1`, `strategy: Recreate`).
  - Opens a single `MongoClient.watch()` cursor across **all** tenant databases using a regex namespace filter (`ns.db: /^vv_/`) so one cursor covers all tenants.
  - On first run (Typesense collection empty), performs a full initial sync by reading all tenant records from MongoDB in batches.
  - Persists the change-stream resume token in the central registry DB (`sync_state` collection) so restarts resume without data loss.
  - Handles `insert`/`update`/`replace` → Typesense upsert, `delete` → Typesense delete, `dropDatabase` → delete all tenant records.

- **MongoDB** (Multiple Instances)
  - **BFF MongoDB** (`MONGODB_URI`): Stores only session information via `connect-mongo`.
  - **Tenant MongoDB Databases** (Dynamic `MONGODB_URI_BASE` + tenantId):
    - Each tenant has a completely separate MongoDB database.
    - Database naming: `vv_<12-char-SHA256-hash-of-tenantId>` (e.g., `vv_a1b2c3d4e5f6`). The tenantId itself remains `user_{githubId}` or `org_{githubId}`; only the MongoDB DB name is hashed (MongoDB's 38-byte name limit).
    - Stores tenant-specific data: users, records, and releases.
    - Complete logical and physical isolation between tenants.
    - Users can be members of multiple organization tenants.
  - **Central Registry** (`MONGODB_REGISTRY_URI`): Stores user-tenant-role mappings, tenant metadata, and sync worker resume token (`sync_state` collection).
  - Running via Docker for development (see `infra/docker-compose.yml`).

- **External APIs**
  - Music metadata APIs (Discogs, MusicBrainz) for barcode lookups.
  - GitHub OAuth for authentication.

## 4. Deployment Architecture

Three active environments exist; RPi 5 remains the production target.

### Local DEV
- Docker Compose + MongoDB on localhost (`infra/docker-compose.yml`).
- Vite dev server on port 3000 (proxies `/graphql` and `/auth` to BFF).
- BFF on port 3001, Backend on port 4000.
- `pnpm dev` starts all services in parallel.

### Staging (Kubernetes)
- Kubernetes cluster, namespace `vinylvault-staging`, managed via kubectl + Kustomize (`infra/k8s/`).
- Kubernetes Secrets hold MongoDB passwords.
- StatefulSets for MongoDB: `mongodb-bff` and `mongodb-backend`.
- Typesense: 3-node Raft StatefulSet (`infra/k8s/base/typesense/`) with headless Service for Raft peer DNS and ClusterIP Service for API queries.
- Sync worker: single-replica Deployment (`infra/k8s/base/sync-worker/`) with `strategy: Recreate`.
- Images pushed to GHCR: `ghcr.io/mloitzl/vinylvault-*:latest-staging`.
- GitHub Actions CI/CD with a self-hosted runner; build/push/deploy scripts in `scripts/`.
- Frontend served as pre-built static assets.

### Demo (Koyeb)
- `packages/demo-server` orchestrator forks Backend (port 4001) and BFF (port 3001) as child processes and proxies `/graphql` + `/auth` routes; listens on port 8080.
- Deployed via `infra/Dockerfile.demoserver`.
- Frontend built with `pnpm build` and deployed to Vercel.
- Build pipeline: `generate-schema.mjs` → `pnpm relay` → `vite build`.
- Full-text search uses **Typesense Cloud** free tier (100k documents). MongoDB Atlas M0 is a replica set, so change streams are supported and the sync worker runs as a separate Koyeb service.
- Required env vars: `TYPESENSE_HOST`, `TYPESENSE_PORT=443`, `TYPESENSE_PROTOCOL=https`, `TYPESENSE_API_KEY`.

### Production Target (Raspberry Pi 5)
- Reverse proxy (Caddy) terminates TLS (Let’s Encrypt) and routes `/graphql` and `/auth/*` to the BFF; serves static frontend assets.
- BFF and Domain Backend run as containers accessible only via the reverse proxy / internal Docker network.
- MongoDB bound to localhost or internal Docker network; not exposed to the internet.
- Security hardening: SSH keys only, firewall restricting inbound to HTTPS.

## 5. Key Cross-Cutting Concepts

### 5.1 Authentication and Sessions

**GitHub OAuth Web Application Flow:**

1. User clicks "Sign in with GitHub" → frontend redirects to `/auth/github`.
2. BFF redirects to GitHub authorization URL with `client_id` and `redirect_uri`.
3. User authorizes on GitHub → GitHub redirects to `/auth/github/callback` with code.
4. BFF exchanges code for access token via GitHub API.
5. BFF fetches user profile from GitHub API.
6. BFF calls Backend GraphQL mutation to create/update user in Backend MongoDB `users` collection.
7. BFF creates session (stored in BFF MongoDB via `connect-mongo`).
8. BFF sets HTTP-only session cookie and redirects to frontend.

**Session Management:**

- Sessions stored in BFF MongoDB `sessions` collection (via `connect-mongo`).
- Cookie: `vinylvault.sid`, HTTP-only, secure in production.
- SameSite: `Lax` (required for OAuth/GitHub App redirects to preserve the session cookie across cross-site callback navigations; `Strict` would drop it). In production, pair with `Secure` (already enabled) and HTTPS termination at the proxy.
- Session contains: `userId`, `githubId`, `username`, `role`, `avatarUrl`, `displayName`, `tenantId`, `tenantType`, `availableTenants`, `createdAt`, `updatedAt`.
- Frontend fetches user and tenant info via `/auth/me` endpoint on load.
- Users can switch tenants via `/auth/switch-tenant` endpoint, updating session context.
- No backend JWT is exposed to the browser.

**Why OAuth-style redirects need at least SameSite=Lax**

When the user returns from GitHub (OAuth) or a GitHub App installation, the browser performs a cross-site top-level navigation back to `/auth/github/callback` or `/auth/setup`. With `SameSite=Strict`, the session cookie is **not** sent on that cross-site navigation, so the callback cannot see `req.session.user` and treats the user as unauthenticated. With `SameSite=Lax`, cookies are sent on top-level GET navigations, which is what OAuth flows use, so the session survives.

Flow (happy path with Lax):

````mermaid
sequenceDiagram
  participant U as User Browser
  participant FE as Frontend/BFF
  participant GH as GitHub (OAuth/App)
  Note over FE: Session cookie set (SameSite=Lax)
  U->>FE: Click "Login" or "Install App"
  FE->>U: 302 to GH (auth/install request)
  U->>GH: Navigate to GitHub
  GH-->>U: Login & consent/install UI
  U->>GH: Approve
  GH->>U: 302 back to FE /auth/.../callback
  U->>FE: Navigate to callback (top-level GET)
  Note over U: Cookie SENT because SameSite=Lax on top-level nav
  FE->>FE: Read session (user present), finalize login/setup
  FE-->>U: Redirect to app with session intact
````

If the provider used a POST redirect (rare), `Lax` would still drop the cookie; the alternative is `SameSite=None; Secure` on HTTPS. For GitHub OAuth and GitHub App installs (top-level GET), `Lax` is sufficient and avoids `None`/third-party semantics.

**Data Flow Between BFF and Backend:**

- BFF never writes directly to any tenant MongoDB database.
- All tenant operations (create, update, fetch) are forwarded from BFF to Backend via GraphQL queries and mutations with tenant context.
- BFF stores only session data in its own MongoDB instance.
- Backend validates tenant context in JWT and routes operations to correct tenant database.

### 5.1.1 Dual Authentication Pattern

The system uses two complementary GitHub authentication modes:

**Mode 1: User Authentication**
- "Sign in with GitHub" button on login screen
- Provides: User avatar, email, GitHub username
- Result: User identification ("I am Martin")
- Scope: `read:user user:email read:org`
- Use: Personal tenant creation and user-scoped operations

**Mode 2: GitHub App Installation**
- "Add Organization" button allowing explicit app installation
- User directed to: `https://github.com/apps/<app-slug>/installations/new`
- GitHub displays only organizations where user has installation authority
- Provides: Installation ID and organization name
- Result: Organization-to-installation association ("Organization X can be managed by Installation ID")
- No additional scopes needed - installation flow grants necessary permissions automatically
- Use: Organization tenant creation with explicit user intent

**Integration Flow:**

1. Logged-in user clicks "Add Organization" button on frontend
2. Frontend redirects to GitHub App installation URL
3. GitHub validates user has installation authority for selected org(s)
4. User completes installation → GitHub sends `installation.created` webhook
5. Backend receives webhook with `installation_id` and `account.login` (org name) to `POST /webhook/github`
6. Backend validates webhook signature and stores installation in `installations` collection (registry)
7. GitHub redirects user to `/auth/setup?installation_id=...` (handled by BFF)
8. BFF verifies user is authenticated and calls backend mutation `completeInstallationSetup`
9. Backend links user to installation in `user_installation_roles` collection
10. Backend creates organization tenant and adds user as ADMIN
11. BFF updates session to include new org tenant in `availableTenants`
12. Frontend automatically switches to new org tenant and displays success message

**Advantages Over Automatic Sync:**
- Permission validation: GitHub filters orgs automatically - only shows ones user can manage
- Privacy: System only learns about orgs where user explicitly installs app
- User intent: Explicit action, not surprising automatic tenant creation
- Database efficiency: Only org tenants created when wanted
- No permission confusion: GitHub guarantees install only if user has authority
- Webhook-driven: Installation events create tenants in real-time, not on login

### 5.2 Authorization

**Role Model:**

- `ADMIN`: Full access within a tenant, can manage users and roles within that tenant.
- `MEMBER`: Can add/update/delete own records within a tenant.
- `VIEWER`: Read-only access to the collection within a tenant.

**Tenant Types:**

- `user`: Personal tenant, one per user. User is automatically ADMIN of their personal tenant.
- `org`: Organization tenant, derived from GitHub organization membership.

**Enforcement:**

- BFF enforces role-based access on GraphQL operations exposed to the client.
- Backend validates and trusts only JWTs signed by the BFF.
- JWT claims: `sub` (user id), `tenantId`, `tenantType`, `role` (within tenant), `iat`, `exp`, optional `github_login`.
- Backend enforces tenant isolation by validating tenantId in all operations.
- JWT secret configured via `JWT_SECRET` environment variable.
- JWT expiration configured via `JWT_EXPIRES_IN` (default: 15 minutes).

### 5.3 Data Model (Logical)

#### Database Architecture Overview

The system uses a multi-tenant database architecture with three separate MongoDB instances/databases:

```mermaid
graph TB
    subgraph "BFF MongoDB"
        SESS[sessions<br/>Session storage]
    end
    
    subgraph "Registry Database<br/>(vinylvault_registry)"
        USERS[users<br/>Global user profiles]
        TENANTS[tenants<br/>Tenant metadata]
        UTR[user_tenant_roles<br/>User-tenant mappings]
        INST[installations<br/>GitHub App installations]
    end
    
    subgraph "Tenant Databases"
        subgraph "vv_a1b2c3d4e5f6 (personal tenant)"
            TREC1[records]
            TREL1[releases]
            TSCR1[scoring_configs]
        end
        
        subgraph "vv_b2c3d4e5f601 (org tenant)"
            TREC2[records]
            TREL2[releases]
            TSCR2[scoring_configs]
        end
    end
    
    BFF[BFF Service]
    BACKEND[Backend Service]
    
    BFF -->|Session management| SESS
    BFF -->|User/tenant lookup| USERS
    BFF -->|User/tenant lookup| TENANTS
    BFF -->|User/tenant lookup| UTR
    
    BACKEND -->|Tenant operations| TREC1
    BACKEND -->|Tenant operations| TREL1
    BACKEND -->|Tenant operations| TSCR1
    BACKEND -->|Tenant operations| TREC2
    BACKEND -->|Tenant operations| TREL2
    BACKEND -->|Tenant operations| TSCR2
    BACKEND -->|Webhook processing| INST
    BACKEND -->|User/tenant management| USERS
    BACKEND -->|User/tenant management| TENANTS
    BACKEND -->|User/tenant management| UTR
    
    style SESS fill:#e1f5ff
    style USERS fill:#fff4e1
    style TENANTS fill:#fff4e1
    style UTR fill:#fff4e1
    style INST fill:#fff4e1
    style TREC1 fill:#e8f5e9
    style TREL1 fill:#e8f5e9
    style TSCR1 fill:#e8f5e9
    style TREC2 fill:#e8f5e9
    style TREL2 fill:#e8f5e9
    style TSCR2 fill:#e8f5e9
```

**Database Responsibilities:**
- **BFF MongoDB**: Session-only storage (`connect-mongo`), isolated from business data
- **Registry Database**: Global user profiles, tenant metadata, user-tenant-role mappings, GitHub App installations
- **Tenant Databases**: Isolated per-tenant data (records, cached releases, optional scoring configs)

#### Entity Relationship Diagram

```mermaid
erDiagram
    %% Registry Database Entities
    USER ||--o{ USER_TENANT_ROLE : "has roles in"
    TENANT ||--o{ USER_TENANT_ROLE : "has members"
    INSTALLATION ||--o{ USER_TENANT_ROLE : "creates"
    
    %% Tenant Database Entities (within each tenant DB)
    USER ||--o{ RECORD : "owns"
    RELEASE ||--o{ RECORD : "describes"
    
    USER {
        ObjectId _id PK
        string githubId UK
        string username
        string displayName
        string email
        string avatarUrl
        date createdAt
        date updatedAt
    }
    
    TENANT {
        ObjectId _id PK
        string tenantId UK
        string tenantType "user|org"
        string name
        string githubOrgName
        string databaseName
        date createdAt
        date updatedAt
    }
    
    USER_TENANT_ROLE {
        ObjectId _id PK
        string userId FK
        string tenantId FK
        string tenantType
        string role "ADMIN|MEMBER|VIEWER"
        date createdAt
        date updatedAt
    }
    
    INSTALLATION {
        ObjectId _id PK
        int installationId UK
        string accountLogin
        string accountType
        int accountId
        date createdAt
    }
    
    RECORD {
        ObjectId _id PK
        ObjectId releaseId FK
        ObjectId userId FK
        date purchaseDate
        float price
        string condition
        string location
        string notes
        date createdAt
        date updatedAt
    }
    
    RELEASE {
        ObjectId _id PK
        string barcode
        string artist
        string title
        int year
        string format
        array genre
        array style
        string label
        string country
        string coverImageUrl
        array trackList
        string externalId
        string source "DISCOGS|MUSICBRAINZ|MANUAL"
        date createdAt
        date updatedAt
    }
```

**Key Relationships:**
- **User ↔ Tenant** (N:M): Users can access multiple tenants (personal + organizations), mapped via `user_tenant_roles`
- **Installation → Tenant** (1:1): Each GitHub App installation creates one organization tenant
- **User → Record** (1:N): Within a tenant, users own multiple records
- **Release → Record** (1:N): A release (cached metadata) can be referenced by multiple physical records

#### Schema Details

**User** (Tenant MongoDB `users` collection)
```
{
  _id: ObjectId,
  githubId: string,
  username: string,
  displayName: string,
  email: string | null,
  avatarUrl: string,
  role: 'ADMIN' | 'MEMBER' | 'VIEWER',  // Role within this tenant
  createdAt: Date,
  updatedAt: Date
}
```

**UserTenantRole** (Central Registry MongoDB `userTenantRoles` collection)
```
{
  _id: ObjectId,
  userId: string,           // User's GitHub ID
  tenantId: string,         // Tenant identifier
  tenantType: 'user' | 'org',
  role: 'ADMIN' | 'MEMBER' | 'VIEWER',
  createdAt: Date,
  updatedAt: Date
}
```

**Tenant** (Central Registry MongoDB `tenants` collection)
```
{
  _id: ObjectId,
  tenantId: string,         // Unique tenant identifier
  tenantType: 'user' | 'org',
  name: string,             // Display name
  githubOrgName: string | null,  // For org tenants
  databaseName: string,     // MongoDB database name for this tenant
  createdAt: Date,
  updatedAt: Date
}
```

**Session** (BFF MongoDB `sessions` collection)
- Managed automatically by `connect-mongo`.
- Contains session ID, expiry, current tenantId, and user data snapshot.

**Record** (Tenant MongoDB `records` collection)
```
{
  _id: ObjectId,
  releaseId: ObjectId,      // Reference to releases collection
  userId: ObjectId,          // Reference to users collection (owner)
  purchaseDate?: Date,
  price?: number,
  condition?: string,        // e.g., "Mint", "Very Good", "Good"
  location?: string,         // Storage location
  notes?: string,            // Personal notes
  createdAt: Date,
  updatedAt: Date
}
```
- Represents a physical vinyl owned by a user within this tenant.
- Links to User and Release within the same tenant database.
- Indexes: userId+createdAt, location, text search on notes/condition/location.

**Release** (Tenant MongoDB `releases` collection)
- Cached metadata about a particular music release from an external source.
- Isolated per tenant to ensure data security.

**Album** (Aggregated from releases at query time)
- Normalized album combining data from multiple release sources.
- Contains: primary release with score breakdown, alternative releases, merged genres/styles, all external IDs.
- Created by the scoring orchestrator during barcode lookup.
- Includes score breakdown for traceability (QR-02): mediaType, countryPreference, trackListCompleteness, coverArt, labelInfo, catalogNumber, yearInfo, sourceBonus.

**Relationships:**
- User 1—N Record (within same tenant)
- Release 1—N Record (within same tenant)
- Album 1—N Release (grouped by normalized artist + title, within same tenant)
- User N—N Tenant (via UserTenantRole mapping)

### 5.4 External Integration

**Barcode Lookup Flow (Implemented with Blended Scoring):**
- Barcode scan from browser (camera via @zxing/browser or manual input) → BFF → Domain Backend.
- Backend extracts tenant context from JWT and connects to appropriate tenant database.
- The backend checks the tenant's local `releases` cache by barcode and returns cached candidates when available.
- If no cached release is found, the backend:
  1. Queries Discogs and MusicBrainz APIs **in parallel** via `orchestrator.ts`.
  2. Converts results to a unified `RawRelease` format.
  3. **Normalizes** text fields (lowercase, trim, remove articles like "The", "A", "An", remove edition affixes like "Remastered", "Deluxe").
  4. **Groups** releases by normalized artist + title into album candidates.
  5. **Scores** each release based on configurable weights:
     - Media type preference (vinyl/LP: +20)
     - Country preference (US/UK/AT/DE: +15, deprioritized RU/CN: -10)
     - Track list completeness (+25 for complete, +10 for partial)
     - Cover art availability (+15)
     - Label info (+10), Catalog number (+5)
     - Discogs source bonus (+5)
  6. **Selects primary release** (highest score, with deterministic tie-breaking by earliest year).
  7. **Aggregates** into `Album` objects with best track list, merged genres/styles, all external IDs.
  8. **Caches results** in the tenant's database for future lookups.
  9. Returns `Album` objects to the BFF with full score breakdowns for traceability.

**Scoring Configuration:**
- Stored in `packages/backend/config/scoring.json`.
- JSON Schema validation via `scoring.schema.json`.
- Configurable without code changes (FR-CF-1, FR-CF-2, FR-CF-3).
- Includes: weights, preferred/deprioritized countries, media types, normalization affixes, source-specific rules.

### 5.5 GraphQL API Design

**BFF GraphQL Schema (`packages/bff/src/schema.graphql`):**
- Contains **only** auth/session types: `viewer` (current user with tenant context), `switchTenant` mutation, `Tenant`, `FeatureFlags`, and `node`/`PageInfo` Relay plumbing.
- All other types (`Record`, `Release`, `Album`, `records` query, CRUD mutations, etc.) come from the Backend schema and are proxied automatically via schema stitching — no manual resolver duplication.
- `createStitchedSchema()` in `packages/bff/src/graphql/schema.ts` reads the static backend SDL file and stitches it with the BFF schema at startup.
- `buildHTTPExecutor` in `packages/bff/src/graphql/executor.ts` injects the JWT into every forwarded request.
- Enums: `TenantType` (USER, ORGANIZATION), `TenantRole` (ADMIN, MEMBER, VIEWER).

**Backend GraphQL Schema (`packages/backend/src/schema.graphql`):**
- Domain-centric schema for internal use by BFF.
- All operations execute within tenant database context (extracted from JWT).
- Queries: `user`, `record`, `release`, `releasesByBarcode`, `records`, `artists`, `albums`, `genres` (all tenant-scoped with cursor pagination and filtering), `userTenants`.
- Mutations: 
  - Barcode & Records: `lookupBarcode` (uses tenant cache), `createRecord`, `updateRecord`, `deleteRecord`
  - User Management: `upsertUser`
  - Tenant Management: `createTenant`, `addUserToTenant`
  - GitHub App Integration: `handleGitHubInstallationWebhook`, `completeInstallationSetup`
- Full `Album` type with detailed scoring information, alternative releases, and external IDs.
- `LookupBarcodePayload` includes `albums[]`, legacy `releases[]` (deprecated), `fromCache`, `timing`, and `errors[]`.
- `ScoreBreakdown` type provides detailed point attribution: `mediaType`, `countryPreference`, `trackListCompleteness`, `coverArt`, `labelInfo`, `catalogNumber`, `yearInfo`, `sourceBonus`.
- `RecordFilter` includes `genre: String` in addition to existing fields.
- GitHub App types: `GitHubInstallationWebhookInput` (with payload and signature), `CompleteInstallationSetupInput` (with userId and installationId).
- Tenant-aware authorization checks for admin-only mutations (tenant management).

**JWT Token Structure:**
- Issued by BFF, validated by Backend.
- Contains: `userId`, `username`, `avatarUrl`, `tenantId`, `tenantRole`.
- Backend extracts `tenantId` to dynamically connect to correct tenant database.
- Backend extracts `tenantRole` for authorization decisions (admin, member, viewer).
- Short-lived (15 minutes) to minimize stale tenant context.

**GitHub OAuth:**
- Browser interacts only with BFF's auth endpoints.
- GitHub tokens and secrets stay server-side.
- Configuration via environment variables:
  - `GITHUB_CLIENT_ID`
  - `GITHUB_CLIENT_SECRET`
  - `GITHUB_CALLBACK_URL`
  - `GITHUB_CALLBACK_URLS` (comma-separated list for multiple environments)

**GitHub App Installation Flow (Phase 6.5):**

*Overview:* Explicit user-driven app installation for organization tenant onboarding.

*Flow Diagram:*
```
┌─────────────────────────────────────┐
│ User clicks "Add Organization"      │
├─────────────────────────────────────┤
│ Redirects to GitHub App install URL │
│ https://github.com/apps/vinyl-...   │
└────────────┬────────────────────────┘
             │
             ↓
┌─────────────────────────────────────┐
│ GitHub displays orgs where user     │
│ has installation authority          │
│ (typically org owners/managers)     │
└────────────┬────────────────────────┘
             │
             ↓
┌─────────────────────────────────────┐
│ User clicks "Install"               │
│ GitHub validates permissions        │
│ (automatic - no access to restricted orgs)
└────────────┬────────────────────────┘
             │
      ┌──────┴───────┐
      ↓              ↓
┌────────────┐   ┌─────────────────┐
│ Webhook    │   │ Browser Redirect│
│ POST       │   │ to /setup       │
│ /webhook/  │   │ with installation
│ github     │   │ _id param       │
└────┬───────┘   └────────┬────────┘
     │                    │
     └────────┬───────────┘
              ↓
      ┌──────────────────┐
      │ Backend Links    │
      │ User to Install  │
      │ Creates Org      │
      │ Tenant           │
      └──────────────────┘
             ↓
      ┌──────────────────┐
      │ Org Tenant Ready │
      │ in Switcher      │
      └──────────────────┘
```

*New Endpoints:*
- `POST /webhook/github` - BFF/Backend webhook endpoint
  - Receives GitHub `installation.created` events
  - Validates webhook signature: `X-Hub-Signature-256` header verified with app webhook secret
  - Extracts: `installation_id`, `account.login` (org name), `account.type`
  - Stores installation record in `installations` collection
  - Returns 200 OK

- `GET /setup?installation_id=...&setup_action=install` - BFF endpoint
  - Requires authenticated user (session cookie validation)
  - Extracts `installation_id` from query parameter
  - Verifies user is authenticated (CSRF protection)
  - Links user to installation in `user_installation_roles` collection
  - Calls Backend to create organization tenant for the installation
  - Adds user as ADMIN role in new org tenant
  - Redirects to app with success message or error page

*New Database Collections (Registry Database):*

`installations` Collection:
```typescript
{
  _id: ObjectId,
  installation_id: number,        // GitHub installation ID (unique)
  account_login: string,          // Organization name (e.g., "acme-corp")
  account_type: 'User' | 'Organization',
  account_id: number,             // GitHub account ID
  repositories_count: number,     // (optional) number of repos
  created_at: Date,               // When app was installed
  updated_at: Date,
  
  // Who installed it
  installed_by_user_id: string,   // Local user ID who performed installation
  installed_at: Date
}
```

`user_installation_roles` Collection:
```typescript
{
  _id: ObjectId,
  user_id: string,                // Local user ID
  installation_id: number,        // Foreign key to installations (GitHub installation ID)
  org_name: string,               // Organization name (denormalized for easy lookup)
  role: 'OWNER' | 'MANAGER',      // User's role at time of installation (from GitHub org API)
  created_at: Date
}
```

*Security Considerations:*
- Webhook signature validation required: Always verify `X-Hub-Signature-256` header against app webhook secret
- Installation ID verification: Verify installation_id from URL matches webhook-received data (optional double-check)
- User authentication required: User must be authenticated (session cookie) to complete `/setup`
- CSRF protection: Validate session state during installation completion
- Rate limiting: Prevent duplicate installation attempts within short timeframe

*Configuration via environment variables:*
  - `GITHUB_APP_ID` - GitHub App application ID
  - `GITHUB_APP_PRIVATE_KEY` - GitHub App private key (PEM format, multi-line)
  - `GITHUB_APP_WEBHOOK_SECRET` - Webhook signature secret for HMAC-SHA256 validation
  - `GITHUB_APP_INSTALLATION_URL` - Installation URL (e.g., `https://github.com/apps/vinyl-vault/installations/new`)

**Environment Configuration:**
- All secrets stored in `.env` file (not committed to git).
- See `docs/GITHUB_OAUTH_SETUP.md` for OAuth setup guide.
- Multi-tenant MongoDB configuration:
  - `MONGODB_URI_BASE` - Base connection string for tenant databases (Backend + sync worker)
  - `MONGODB_REGISTRY_URI` - Connection string for registry database (Backend + sync worker)
  - `MONGODB_URI` - Connection string for BFF session storage
- External API tokens: `DISCOGS_API_TOKEN`, `MUSICBRAINZ_USER_AGENT`.
- Typesense: `TYPESENSE_HOST`, `TYPESENSE_PORT`, `TYPESENSE_PROTOCOL`, `TYPESENSE_API_KEY`.

### 5.6 Full-Text Search (Typesense)

**Overview:**

Faceted full-text search is powered by [Typesense](https://typesense.org) — an open-source, typo-tolerant search engine with native 3-node Raft clustering. It replaces a previous MongoDB Atlas Search implementation (which hit the 3-index free-tier limit).

**Collection schema (`records`):**

| Field | Type | Facet | Notes |
|---|---|---|---|
| `id` | string | — | MongoDB `_id.toString()` — globally unique |
| `tenantId` | string | ✓ | Always filtered server-side; never from the browser |
| `releaseArtist` | string | ✓ | |
| `releaseTitle` | string | ✓ | |
| `releaseLabel` | string | — | Searchable |
| `releaseGenre` | string[] | ✓ | |
| `releaseStyle` | string[] | — | Searchable |
| `releaseTrackTitles` | string[] | — | Searchable |
| `releaseFormat` | string | ✓ | LP, 7", CD, etc. |
| `releaseCountry` | string | ✓ | |
| `releaseYear` | int32 | ✓ | |
| `condition` | string | ✓ | Mint, VG+, etc. |
| `location` | string | ✓ | Storage shelf/box |
| `notes` | string | — | Searchable |
| `updatedAt` | int64 | — | Unix ms; default sort field |

**Search flow (two-step):**
1. Backend calls `typesenseService.search()` with the user query, active facet filters, and a mandatory `filter_by: tenantId:=<id>` clause.
2. Typesense returns ranked document IDs, highlight snippets, and facet counts.
3. Backend fetches full `RecordDocument` objects from MongoDB by those IDs, preserving rank order.
4. Response follows the same `SearchRecordsResult` GraphQL shape as before — frontend and BFF have zero changes.

**Query syntax supported:**
- Plain words: `Beatles Abbey Road` — typo-tolerant (≥5-char tokens get 1 edit allowed)
- Quoted phrase: `"dark side of the moon"` — exact phrase match
- `+term` — term is required (must appear)
- `-term` — term is excluded (dropped; Typesense has no cross-field NOT operator in `q`)

**Tenant isolation:**
- The backend always appends `filter_by: tenantId:=\`<id>\`` before executing any Typesense query.
- Facet filter values are backtick-quoted to handle special characters (e.g. `Folk, World, & Country`).
- No per-tenant collections or API keys are needed.

**Sync worker:**
- A separate process (`packages/backend/src/sync-worker/`) keeps Typesense in sync via a single `MongoClient.watch()` cursor filtered to `ns.db: /^vv_/`.
- Resume token is stored in the registry DB `sync_state` collection for crash-safe restarts.
- Initial sync runs automatically when the Typesense collection is empty.
- `scripts/admin/typesense-full-sync.mjs` can wipe and re-index from scratch (for disaster recovery or schema changes).

**Deployment topology:**

| Environment | Typesense | Sync Worker |
|---|---|---|
| Local dev | Single-node Docker container (port 8108) | Docker service |
| k3s staging/prod | 3-node Raft StatefulSet, headless Service for peer DNS | `replicas: 1`, `strategy: Recreate` Deployment |
| Demo (Koyeb) | Typesense Cloud free tier (100k docs) | Separate Koyeb service |

## 6. Quality Attributes

- **Security:**
  - JWT between BFF and backend only.
  - HTTPS, secure cookies (HTTP-only, SameSite=lax).
  - Minimal exposure of internal ports.
  - Hardened OS and MongoDB configuration.
  - Environment secrets never exposed to frontend.

- **Maintainability:**
  - TypeScript strict mode across all services.
  - pnpm monorepo with shared configurations.
  - Clear separation between BFF and domain backend.
  - Shared tsconfig base configuration.

- **Performance:**
  - Local MongoDB and caching of release metadata.
  - Lightweight traffic profile, suitable for RPi 5.
  - Vite for fast frontend development.

- **Availability:**
  - Designed for single-node deployment.
  - Docker Compose for service orchestration.
  - Can be containerized for cloud deployment later.

## 7. Project Structure

```
vinylvault/
├── packages/
│   ├── frontend/          # React + Relay + Vite
│   │   └── src/
│   │       ├── components/    # UI components (Header, ScanBarcode, etc.)
│   │       ├── contexts/      # React contexts (AuthContext)
│   │       ├── hooks/         # Custom hooks
│   │       ├── pages/         # Page components (Collection, Browse, etc.)
│   │       ├── relay/         # Relay environment configuration
│   │       └── types/         # TypeScript type definitions
│   ├── bff/               # Express + Apollo Server (schema-stitched)
│   │   └── src/
│   │       ├── auth/          # GitHub OAuth handlers, JWT utilities
│   │       ├── config/        # Environment configuration
│   │       ├── db/            # MongoDB connection
│   │       ├── graphql/       # Stitched schema (schema.ts, executor.ts)
│   │       └── types/         # TypeScript types (context, session)
│   ├── backend/           # Apollo Server (standalone)
│   │   ├── config/        # Scoring configuration
│   │   │   ├── scoring.json
│   │   │   └── scoring.schema.json
│   │   └── src/
│   │       ├── db/            # MongoDB connection and collections
│   │       ├── graphql/       # GraphQL schema and resolvers
│   │       ├── models/        # Data models
│   │       ├── services/      # Business logic
│   │       │   ├── typesense.ts           # Typesense client, upsert/delete/search
│   │       │   ├── typesense-query.ts     # Query builder (phrase/must/mustNot/facets)
│   │       │   ├── typesense-highlights.ts # <mark> snippet → SearchHighlight[]
│   │       │   ├── discogs.ts             # Discogs API client
│   │       │   ├── musicbrainz.ts         # MusicBrainz API client
│   │       │   ├── releasesCache.ts       # Release caching
│   │       │   ├── users.ts               # User management
│   │       │   └── scoring/               # Blended scoring module
│   │       │       ├── types.ts
│   │       │       ├── config.ts
│   │       │       ├── normalize.ts
│   │       │       ├── score.ts
│   │       │       ├── aggregate.ts
│   │       │       ├── orchestrator.ts
│   │       │       └── index.ts
│   │       └── sync-worker/   # MongoDB→Typesense change-stream sync worker
│   │           ├── index.ts                  # Entry point
│   │           ├── change-stream-listener.ts # watch() cursor + event handlers
│   │           ├── initial-sync.ts           # Full initial sync across all tenants
│   │           └── token-store.ts            # Resume token persistence (registry DB)
│   └── demo-server/       # Koyeb orchestrator (forks BFF + Backend, port 8080)
├── scripts/
│   └── admin/             # Admin management scripts
│       ├── typesense-full-sync.mjs  # Wipe + re-index all records in Typesense
│       └── ...                      # list-users, add-user-to-tenant, etc.
├── infra/
│   ├── docker-compose.yml     # Local DEV service orchestration
│   ├── Caddyfile              # Reverse proxy configuration
│   ├── Dockerfile.bff         # BFF container
│   ├── Dockerfile.backend     # Backend container
│   ├── Dockerfile.frontend    # Frontend container
│   ├── Dockerfile.demoserver  # Demo server container (Koyeb)
│   ├── Dockerfile.sync-worker # Sync worker container
│   └── k8s/               # Kubernetes manifests (Kustomize, staging)
│       └── base/
│           ├── typesense/         # 3-node Raft StatefulSet + headless Service
│           └── sync-worker/       # Sync worker Deployment (replicas: 1, Recreate)
├── docs/                  # Setup guides
├── generate-schema.mjs    # Merges BFF + Backend SDL → packages/frontend/schema.graphql
├── .env                   # Environment variables (not in git)
├── pnpm-workspace.yaml    # Monorepo configuration
└── tsconfig.base.json     # Shared TypeScript config
```