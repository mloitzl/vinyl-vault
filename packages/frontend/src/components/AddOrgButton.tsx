...
export function AddOrgButton({ className = '' }: AddOrgButtonProps) {
  ...
  if (isLoading) {
    return (
      <button
        disabled
        className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors opacity-50 cursor-not-allowed ${className}`}
      >
  ...
  if (error && !installUrl) {
    return (
      <button
        disabled
        title={error}
        className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium bg-red-50 text-red-700 opacity-50 cursor-not-allowed ${className}`}
      >
